<script>
    function otraPagina() { //da un mensaje cuando se inscribe correctamente
...
if ((empty($_SESSION["email"]))) {
    ...
    if (isset($_POST['submit'])) { //toma los datos del formulario registro para subirlo a la base de datos y tirar los mensajes de error
        ...
        if (strlen($_POST['Procesador']) > 1 && strlen($_POST['RAM']) > 0 && strlen($_POST['MotherBoard']) > 1 &&  strlen($_POST['Zocalos']) > -1 && strlen($_POST['HDD']) > 1 && strlen($_POST['Marca']) > 0 && strlen($_POST['DIMMs']) > 1 && strlen($_POST['Zocalos_Libres']) > 0 && strlen($_POST['Laboratorio']) > 0) { //verifica que los campos no esten vacios
